...
List of outstanding issues:
    - sample_predictions terrifyingly slow
    - fix sample_predictions_tf
    - sample predictions by calculating the pdf associated to each value, summing those - check?
...
if __name__ == "__main__":
    ...
    .from_tensor_slices((X_full, sample_profiles_logged)) \
    .shuffle(N).batch(N)
    ...
    while training_bool:
        ...
        mse_error_profiles = tf.losses.MSE(ttp_logged[0],profile_sample)
        ...
        if i % print_every == 0:
            logging.info('Epoch {}/{}: Elapsed Time: {}; loss {}, Epochs since best loss: {}; MSE: {}; overlap: {}'.format(i, EPOCHS,datetime.now() - train_start, losses[-1],counter,mse_error_profiles, overlap_ratio))       
        i = i+1
    ...
    for i in range(n_test_profiles):
        ...
        for j in range(1):
            plt.plot(t_associated_r[i],profile_sample, label = "Sample {}".format(j))
            plt.legend()
