...
def train_model(model_nodes, optimizer, dataset,initial_r,
                ...
                min_train_epochs = 100, start_parameters = {}, roll_mean_length = 5,lr = 1e-4,
                norm = True):
    '''
    ...
    print_every = np.max([1, num_epochs/100])
    ...
    rolling_mean_length = roll_mean_length
    ...
    logging.info("# Training Profiles: {}".format(len(initial_r)-len(X_tt)))
    ...
    while training_bool:
        for train_x, train_y in dataset:
            with tf.GradientTape() as tape:
                ...
                sample, mixtures = generate_vector_gauss_mixture(initial_r,pi_,mu_,var_)#generate_tensor_mixture_model(rs,pi_,mu_,var_)    
        ...
        sample_preds, sample_mixtures = generate_vector_gauss_mixture(t_a_r,pi_tt,mu_tt,var_tt)#generate_tensor_mixture_model(rs,pi_tt,mu_tt,var_tt)
